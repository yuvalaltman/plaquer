...
def read_img(filename, color_fluo={}):
    ...
    if np.isin(channels, list(color_fluo.values())).all():
        rgb_order = [color_fluo[metadata.channels[i].channel.name]
...
FONT_KW = {"plot_text_small" : {"fontname": "serif",
                                "weight": "normal",
                                "size": "10",
                                "style": "normal"}}
...
def img_nms(df_preds, iou_threshold=0.5):
    nms_inds = torchvision.ops.nms(_boxes_convert(df_preds),
...
def _compute_ioa_matrices(preds, targets):
    ioa_matrix, _ = _box_ioa_numpy(_boxes_convert(preds),
                                   _boxes_convert(targets))
    label_match_matrix = (targets["class_id"].values.flatten()[:, None]
...
def calculate_img_area_thr(preds, area_alpha_med=AREA_ALPHA_MED):
  return area_alpha_med * preds.groupby("img_large")["area"].median()
...
def remove_abnormal_boxes_area(preds, area_alpha_med=AREA_ALPHA_MED, tol=ABR_TOL):
  ...
  img_area_thr = calculate_img_area_thr(preds, area_alpha_med)
...
def remove_abnormal_boxes(preds,
                          aspect_ratio=ASPECT_RATIO,
                          area_alpha_med=AREA_ALPHA_MED,
                          tol=ABR_TOL):
  preds = remove_abnormal_boxes_aspect_ratio(preds, aspect_ratio=aspect_ratio, tol=tol)
  preds = remove_abnormal_boxes_area(preds, area_alpha_med=area_alpha_med, tol=tol)
  return preds
...
# 5. STITCHING
print("\n5/7 STITCHING ...")
ensemble_preds_dict = get_ensemble_preds_per_img(IOU_NMS,WBF,
                                                 IOU_WBF,
                                                 SKIP_BOX_THRESHOLD,
                                                 IOA,
                                                 CBR_CLASS_AGNOSTIC,
